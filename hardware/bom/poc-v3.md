...
ข้อ 6  ล้อนอก 160 + 93.81 = 253.81 <= wheel_v_max   ->  rpm >= 19.4
ข้อ 7  ล้อใน  160 - 93.81 =  66.19 >= deadband
       ที่ 15% duty                                  ->  rpm <= 33.7
```

```text
หน้าต่าง                 19.4 – 33.7 RPM
เลือก                    25 RPM
...
⚠️ Design spec §7.1 และ §8.3 พิมพ์หน้าต่างนี้ไว้ที่ 18.6–28.6 RPM — เลขนั้นคำนวณที่
`v_max = 150 mm/s` ก่อนที่ design decision S7 จะปรับ `v_max` เป็น 160 mm/s และไม่เคย
คำนวณใหม่ให้ตรงกัน spec เป็นบันทึกการตัดสินใจ ณ ตอนที่ตัดสินใจไว้เฉย ๆ — เอกสารนี้
(BOM) เป็นเอกสารที่ใช้สั่งของจริง หน้าต่างที่ใช้งานคือ 19.4–33.7 RPM ด้านบน
